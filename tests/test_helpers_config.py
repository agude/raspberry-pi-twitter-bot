...
def build_fake_config(home, path, tmpdir):
    CONT = '{ "TEST": ["TEST", "TEST"] }'
...
def test_load_config_XDG_CONFIG_HOME_set(tmpdir):
    home = build_fake_config("home2", ".config/rpi-twitter/config", tmpdir)
...
def test_load_config_XDG_CONFIG_HOME_unset(tmpdir):
    home = build_fake_config("home3", ".config/rpi-twitter/config", tmpdir)
    result = load_config()
    loaded_dict_test(result)


def test_load_config_XDG_CONFIG_HOME_nonstandard(tmpdir):
    home = build_fake_config("home4", "config/rpi-twitter/config", tmpdir)
    os.environ["XDG_CONFIG_HOME"] = os.path.join(home, "config")
    result = load_config()
    loaded_dict_test(result)
...
def test_load_config_rc_file(tmpdir):
    home = build_fake_config("home5", ".rpitwitterrc", tmpdir)
    result = load_config()
    loaded_dict_test(result)
